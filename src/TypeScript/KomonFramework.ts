...
module KomonFramework {
    export var Config = {
        WebApiBaseAddress: "http://km.tns-sifo.se/",
        //WebApiBaseAddress: "http://localhost:35769/",
        WebApiServiceName: "CampaignService"
    };
}

module Site {
    export var Config = {
        //DateFormatDatePicker: "yy-mm-dd",
        LanguageCode: "se"
    };
}
...
module KomonFramework {
    ...
    export class KnockoutHelper {
        ...
        static KnockoutMapByIdMappingOption(): any {
            var mapping = {
                'children': {
                    key: function (data) {
                ...
                'ignore': ["Creatives.Data"]
            };

            return mapping;
        }

        static InitDatePickerBinding(): void {
            ko.bindingHandlers.datepicker = {
                ...
                update: function (element, valueAccessor) {
                    ...
                    var current = <any>$el.datepicker("getDate");
    ...
    export class Client {
        ...
        PostServer<T1, T2>(name: string, arguments: Array<ServerArgument>, controllerUrl: string = null): P.Promise<Komon.Contract.Base.KomonResult<T1, T2>> {

            var url = controllerUrl != null ? this._serverUrl + controllerUrl + name
                : this._serverUrl + "komonapi/I" + this._serviceName + "/" + name;

            var dtoObject = {};
            if (controllerUrl !== null) {
                // If we use a custom controller we must send the arguments in a custom format
                arguments.forEach(a => dtoObject[a.Key] = a.Value);
            }
            else {
                // Komon webapi expects the values to be stringified
                arguments.forEach(a => a.Value = JSON.stringify(a.Value));
            }

            var deferred = defer<Komon.Contract.Base.KomonResult<T1, T2>>();
            //$.post(url, controllerUrl !== null ? dtoObject : JSON.stringify(arguments)).done(a => deferred.resolve(this.CustomizeJson<T1, T2>(a)));

            var jqxhr = $.post(url, controllerUrl !== null ? dtoObject : JSON.stringify(arguments))
                .done(a => deferred.resolve(this.CustomizeJson<T1, T2>(a)))
                .fail(a => deferred.reject(new Error()));
        ...
        CustomizeJson<T1, T2>(json: any): Komon.Contract.Base.KomonResult<T1, T2> {

            // Uses custom extended .parseJson from "jquery.parsejson.js" to handle dates correctly
            var result = $.parseJSON(JSON.stringify(json), true);

            return {
                Result: <T1>result,
                ObservableResult: function (option) {
                    if (typeof option === "undefined") { option = KomonFramework.KnockoutHelper.KnockoutMapByIdMappingOption(); }

                    return <T2>ko.mapping.fromJS(result, option);
                }
            };
        }
...
module Komon.Contract.Base {
    // Add ToObservable to new objects
    export class JsonObjectBase<TObservable> {

        // Overload extension method toKoObservable on objects to with TObservable Info
        ToKoObservable(option: any = {}): TObservable {
            return MapToObservable(this, option);
        }
    }

    // Helper method ToObservable
    function MapToObservable(that: any, option: any): any {
        return ko.mapping.fromJS(that, option);
    }

    export interface KomonResult<TClass, TObsClass> {
        Result: TClass;
        ObservableResult(option?: any): TObsClass;
    }

    export interface IBaseViewModel {
        InitViewModel(args: string[]): void;
        Client: any;
    }
}
